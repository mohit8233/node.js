...
const fetchEmployee = async () => {
  ...
  const handleDelete = (id) => {
    ...
    deleteEmployee(id)
      .then((deleteApi) => {
        if (deleteApi.data.status) {
          alert(deleteApi.data.message || "Employee Deleted");
          refreshData();
          fetchEmployee();
        } else {
          alert(deleteApi.data.message);
        }
      })
      .catch((error) => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-center items-center mb-10">
          ...
          <select
            value={params.sortBy}
            onChange={(e) =>
              setParams({ ...params, sortBy: e.target.value, page: 1 })
            }
          ...
          <select
            value={params.order}
            onChange={(e) =>
              setParams({ ...params, order: e.target.value, page: 1 })
            }
